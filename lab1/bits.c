...
int isLessOrEqual(int x, int y) {
  // if x is negative, we know for sure it is true (1)
  // if y is negative, we know for sure it is false (0)
  int xNeg = (x >> 31) & 0xF; // 0 if pos, 1 if neg
  int yNeg = (y >> 31) & 0xF; // 0 if pos, 1 if neg
  int x_ls_y = ((y + (~x + 1)) >> 31) + 1; // 0 if x > y, 1 if x <= y
  return (x_ls_y | (xNeg & (!yNeg))) & (!((!xNeg) & yNeg));
}
/* 
 ...
 */
int absVal(int x) {
  int xNeg = (x >> 31); // 1's if negative, 0's if positive
  return (xNeg & (~x + 1)) | (~xNeg & x);
}
/*
